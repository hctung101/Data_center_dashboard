...
#file.copy("/home/kent/gpuming_templatev13.xlsx", paste0("/data/kent/sparkpool/", filename))
...
for(wallet.i in seq_len(length(wallet))){
  ...
  switch( type.l[wallet.i],
          ...
          TW = {
            ...
            #if(wallet.i==6){
            saveRDS(out1.data, paste0("/data/kent/sparkpool1540/", gsub("-| |:", "", as.Date(Sys.time())), "all_", Mine.name[wallet.i], ".RDS" ), version =  2) 
            ...
            #}
            file.all <- tail( list.files("/data/kent/sparkpool1540", pattern = paste0("all_", Mine.name[wallet.i]), full.names = T), 30)
            ...
            payments <- payments[payments$paid > 0 & payments$time %in% format(seq(Sys.Date() - 30,Sys.Date(),"day"), "%m/%d"), ]
            ...
            payments2 <- data.table(payments)[, .(time = format(seq(Sys.Date()-30,Sys.Date(),"day"), "%m/%d"))]
            ...
            paymentslast <- ifelse(any(ethtransactions$status == "IN" &  grepl(Sys.Date(), ethtransactions$Date)), as.numeric(strsplit(ethtransactions[ethtransactions$status == "IN", ][1, "Value"], " ")[[1]][1]), NA)
          ...
          CN = { 
            ...
            payments <- payments[payments$time %in% format(seq(Sys.Date()-30,Sys.Date(),"day"), "%m/%d"),]
            # For 1/7 pay two time----------------------------------------
            payments.temp.date <- payments$time[duplicated(payments$time)]
            payments.temp <- data.table(payments)[, sum(paid), by = time]
            payments <- payments[!duplicated(payments$time),]
            ...
            payments <- payments[ unlist(sapply(format(seq(Sys.Date(),Sys.Date()-30,"-1 day"), "%m/%d"), function(x){which(payments$time==x)})), ]
            #-----------------------------------------------------------
            total.payments <- data.frame(n1 = c("ETH(15:40) exchange rate", "TotalPaid", "US dollars"),
            ...
            paymentslast <- ifelse(any(ethtransactions$status == "IN" &  grepl(Sys.Date(), ethtransactions$Date)), as.numeric(strsplit(ethtransactions[ethtransactions$status == "IN", ][1, "Value"], " ")[[1]][1]), NA)
            if(wallet.i == 3){
              paymentslast <- ifelse(any(ethtransactions$status == "IN" &  grepl(Sys.Date(), ethtransactions$Date)),  sum(as.numeric(gsub(" Ether", "", ethtransactions[ethtransactions$status == "IN"&  grepl(Sys.Date() - 1, ethtransactions$Date), ][, "Value"]))), NA)}
            summary.data <- rbind(summary.data , c(workers.all.n, online.rig.n, workers.all.n - online.rig.n, percent(online.perc),  paymentslast, total.payments[2,2], Average.hash.rate, Average.per.rig.payment, Average.per.hash.payment, ethrate, offline.rig.name, ethwallet))
          },
          #For EU Data Center----------------------------------------------------------
          EU = { 
...
saveRDS(as.data.frame.matrix(summary.data), paste0("/data/kent/sparkpool1540/", gsub("-| |:", "", as.Date(Sys.time())), "summary.RDS" ), version =  2) 
...
#addDataFrame( t( summary.data.today.last), sheet =  getSheets(wb)[["Summary"]], col.names=F, row.names=F, startRow = 14)
...
#                        CC = "Angela2_Lin@asus.com;Elaine1_Chang@asus.com",	
...
#rawmail <- readLines("/home/kent/email.txt", encoding = "utf-8")
